...
@compile_mode("script")
class AtomicDipolesMACElia(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
...
class AtomicDipolesShiftMACE(AtomicDipolesMACElia):
    ...
    def __init__(self, **kwargs):
        ...
        self._es_shift = torch.nn.Parameter(torch.ones(3))
    ...
    def forward(
        ...
        # output["dipole"] += shift
        output["dipole"] += self._es_shift
        return output

@compile_mode("script")
class AtomicDipolesMACEliaMTP(torch.nn.Module):
    ...
    def forward(
        ...
        a = torch.column_stack((charges.unsqueeze(1),data["delta_positions"]))
        ...
        # total_dipole += fixed_charges 
        total_dipole += self.mtp_shift(total_dipole)
        total_dipole += broke_transl_sym

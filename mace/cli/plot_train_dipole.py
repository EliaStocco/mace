...
def plot(data: pd.DataFrame, min_epoch: int, output_path: str) -> None:
    ...
    ax.grid(True, which="minor", axis="both", linestyle=':', linewidth=0.5, color='black')
    ax.legend()

    ax = axes[1]
    ax.plot(
        valid_data["epoch"],
        valid_data["mae_mu"]["mean"],
        color=colors[1],
        zorder=1,
        label="MAE dipole [eang]",
    )
    ax.fill_between(
        x=valid_data["epoch"],
        y1=valid_data["mae_mu"]["mean"] - valid_data["mae_mu"]["std"],
        y2=valid_data["mae_mu"]["mean"] + valid_data["mae_mu"]["std"],
        alpha=0.5,
        zorder=-1,
        color=colors[1],
    )
